 ...
 * @size: size specifier
 *
 * Return: number of characrtes printed
 */
int handle_write_char(char c, char buffer[], int flags,
		int width, int precision, int size)
{
/* char is stored in left and padded at buffer's right*/
	int i = 0;
	char padd = ' ';
		...
		int prec, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[i], i - 1) + write(1, &buffer[ind], length));
		}
 ...
 * @ind: index at wich the number starts in
 * the buffer
 ...
 * @buffer: buffer array to handle
 * @ind: index at which num starts at buffer
 * printing
 * @length: number length
 * @width: width
 * @flags: calculats active flags
